...
// ==================================================
// В мобільних бравзерах не має подій миші, а є тапи (коли доторкаємося до сенсору пальцем)
// Є 6 подій:

// touchstart - при доторку
// touchmove - при зміщенні пальця
// touchend - при відірванні пальця
// touchenter - при веденні пальця по екрану і наїжджанні на обєкт, на якому навішена подія
// touchleave - палець продовжив скользити і пішов з обєкта, на якому навішена подія
// touchcancel - палець виходить за межі бравзера
...
//Три головні властивості при роботі з мобільними пристроями:
// toushes - видає список пальців, які взаємодіють з екраном
// targetToushes - список пальців, які взаємодіють з конкретним елементом
// chengedToushes - список пальців, які виконали певну назначену дію




// Приклад, як прописувати до події
// window.addEventListener('DOMContentLoaded', () => {
//     const box = document.querySelector('.box');
...
//         console.log('start ');
//         console.log(e.toushes); //видає touchlist з кількістю 
//     });
// });
...
// На основі цих комбінацій можна створювати власні події.
//Н-ад, потрібно відстежити рух, тоді:
// window.addEventListener('DOMContentLoaded', () => {
//     const box = document.querySelector('.box');
...
//         console.log(e.targetTouches[0].pageX); //відстеження координат. н-ад, координати виходь за якісь межі т відбувається переключення слайду в слайдері
//     });
// });
...
// !!Є готові бібліотеки для мобільних подій, н-ад:
...
// Завантаження скриптів на сторінці:
// Коли index.html попадає в бравзер, бравзер ідентифікує сторінку і починає її загрузку
// Бравзер дивиться, що це html з певною мовою і починає завантажувати head
// Підвантажує метатеги і встановлює їх на сторінці, генерує title
// Підгружає стилі: шрифти, css-файли
// Після head переходить до body, і будує верстку тег за тегом
// Вкінці, якщо є script, то бравзер підгружає скриптову структуру
// На цьому підгрузка сторінки закінчена

// Якщо ставити script в head, то отримуємо 2 проблеми: 
// 1. Знизу сторінка ще не побудована, а скрипт запустився. Якщо в скрипті є взаємодія зз елементами, 
// яких ще немає на сторінці, то скрипт не спрацює, буде помилка. Вирішується методом DOMContentLoaded
// 2. При загрузці скрипт блокує сторінку, тобто коли бравзер завантажує структуру, то він спочатку чекає, поки 
// скрипт повністю загрузиться і на цьому місці зависає. Якщо скрипт обємний, або сервер на якому лежить скрипт підвисає, то
// сторінка далі не буде завантажуватися. Цю проблему не можемо вирішити.



// Два атрибута, які вирішують проблеми вище:
// defer - прописуємо як звичайний html-атрибут <script defer src="js/script.js></script>"
// він каже, що бравзер має завантажувати структуру і запустити скрипт у фоновому режимі
// Після чого запустити скрипт, коли він завантажеться. Скрипти з defer не блокують сторінку. 
// Прописуючи defer вказуємо порядковість загрузки скриптів. 
// І можна поставити скрипт в head, - але гугл спід всеодно сваритиметься

// async - <script async src="js/script.js></script>"
// сторінка не чекає асинхронних скриптів. Вміст обробляється і відображається.
// Події DOMContentLoaded і асинхронні скрипти не чекають один одного.
// хто перший загрузився, той і починає працювати.
// Потрібні, коли підключаємо сторонні скрипти, наприклад метрики


// Динамічний скрипт підключаємо у файлі зі скриптом
// function loadScript(src) {
//     const script = document.createElement('script');
//     script.src = src;
//     script.async = false; //false робить строге виконання скрипта один за одним
...
//   38. Робота з Датами
// ==================================================
// const now = new Date('2021-11-20'); //створюємо нову дату і поміщаємо її в змінну now
// console.log (now);

// const now = new Date(2020, 5, 1, 20); //місяці в даті рахуються з нуля
// console.log (now);


//можна передати мілісекунди
// const now = new Date(0); //передаєм 0 мілісекунд, дата стартує від 1970року. Якщо потрібно вказати до 1970, ставим -
// console.log (now);


//Методи Data
//1.Отримування компонента, методи-гетери
const now = new Date(); 
// console.log (now.getFullYear());
// console.log (now.getMonth());
// console.log (now.getDate());
// console.log (now.getDay()); //нумерація з нуля. 0 - неділя, 6 день тижня - субота

// //дані методи повертають значення в місцевому часовому поясі. щоб змінити - додаємо UTC
...
//2.Встановити дату , методи-сетери 
// console.log(now.setHours(18));
// console.log(now);


//Дати можна використовувати для виміру проміжку часу
let start = new Date();

for (let i = 0; i < 100000; i++) {
    let some = i ** 3;  //оператор ** приводить до степені
}
